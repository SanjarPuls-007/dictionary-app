...
class UploadController {
	static uploadFile = async (req, res) => {
		...
		if (uploadFile.mimetype.startsWith('image/')) {
			file_name =
				'Dictionary/images/' + v4() + path.extname(uploadFile.originalname)
		}
		if (uploadFile.mimetype.startsWith('video/')) {
			file_name =
				'Dictionary/videos/' + v4() + path.extname(uploadFile.originalname)
		}
		if (uploadFile.mimetype.startsWith('audio/')) {
			file_name =
				'Dictionary/audio/' + v4() + path.extname(uploadFile.originalname)
		}
	...
	// 		if (!filePath) {
	// 			throw new HttpException(StatusCodes.BAD_REQUEST, 'File path is required')
	// 		}

	// 		await deleteFileS3(filePath)
	// 		await SaveFileModel.deleteOne({ file_path: filePath })
